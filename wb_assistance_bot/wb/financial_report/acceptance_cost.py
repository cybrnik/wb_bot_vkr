...
class Acceptance:
    ...
    def get_acceptance_cost(self, date_from=None, date_to=None):
        params = {}
        if date_from is not None:
            params["dateFrom"] = date_from
        if date_to is not None:
            params["dateTo"] = date_to
        response = self.__get_request(prefix="acceptance_report", params=params)
        response = response.json()
        taskID = response['data']['taskId']
        while True:
            response = self.__get_request(prefix=f"acceptance_report/tasks/{taskID}/status", params=params)
